...
else if(isset($_GET['keyword']))
{
	...
	$query = "SELECT * FROM $tableName WHERE code LIKE '%$keyword%'";
}
else
{
...
if(isset($_GET['status']))
{
	...
	if(isset($_GET['keyword']))
	{
		...
		$query = "SELECT * FROM $tableName WHERE status=$status AND code LIKE '%$keyword%' ORDER BY id DESC LIMIT $start, $limit";
	}
...
else if(isset($_GET['keyword']))
{
	...
	$query="SELECT * FROM $tableName WHERE code LIKE '%$keyword%' ORDER BY id DESC LIMIT $start, $limit";

}
else
{

	$query="SELECT * FROM $tableName ORDER BY created DESC LIMIT $start, $limit";

}

$redeem_result=mysqli_query($mysqli,$query);


$sql_verify="SELECT varify_u.*, user.`name`, user.`email` FROM willdev_verify_user varify_u, willdev_users user WHERE varify_u.`user_id`=user.`id` AND varify_u.`status`='0' ORDER BY varify_u.`id` DESC";
			...
			<div class="page_title_block">
				...
				<div class="col-md-7 col-xs-12">              
					<div class="search_list">
						...
						<div class="add_btn_primary"> <a href="add_redemption.php?add=yes&redirect=<?=$redirectUrl?>">Add Redemption Code</a> </div>
					</div> 
				</div>
				...
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover">
						...
						<tbody>
							<?php
							$i=0;
							if(mysqli_num_rows($redeem_result) > 0)
							{
								while($redeem_row=mysqli_fetch_array($redeem_result))
								{

									$code = !empty($keyword)?highlightWords($redeem_row['code'], $keyword):$redeem_row['code'];
                                    ?>
									<tr>
										<td> 
											<div>
												<div class="checkbox">
													<input type="checkbox" name="post_ids[]" id="checkbox<?php echo $i;?>" value="<?php echo $redeem_row['id']; ?>" class="post_ids">
													<label for="checkbox<?php echo $i;?>"></label>
												</div>
											</div>
										</td>
										<td><?php echo $redeem_row['created']; ?></td>
										<td><?php echo $code;?></td>   
										<td><?php echo $redeem_row['period']; ?></td>
										<td><?php echo $redeem_row['period_length']; ?></td>
										<td>
										    <?php echo ($redeem_row['no_ads'] == 1) ? 'No Ads' : ''; ?>
										    <?php echo ($redeem_row['no_ads'] == 1 && $redeem_row['premium_servers'] == 1) ? ', ' : ''; ?>
										    <?php echo ($redeem_row['premium_servers'] == 1) ? 'Premium Servers' : ''; ?>
										</td>
										<td><?php echo $redeem_row['redeem_count']; ?></td>
										<td>
											<?php if($redeem_row['status']=="0"){?>
												<span class="badge badge-danger badge-icon"><i class="fa fa-check" aria-hidden="true"></i><span>Inactive</span></span>

											<?php }else{?>
												<span class="badge badge-success badge-icon"><i class="fa fa-check" aria-hidden="true"></i><span>Active </span></span>
											<?php }?>
										</td>
										<td nowrap="">
											<a href="add_redemption.php?id=<?php echo $redeem_row['id'];?>&redirect=<?=$redirectUrl?>" class="btn btn-primary" data-toggle="tooltip" data-tooltip="Edit"><i class="fa fa-edit"></i></a>

											<a href="javascript:void(0)" class="btn btn-danger btn_delete_a" data-id="<?php echo $redeem_row['id'];?>" data-toggle="tooltip" data-tooltip="Delete"><i class="fa fa-trash"></i></a>
										</td>
									</tr>
									<?php	
									$i++;
		...
		function(isConfirm) {
			if (isConfirm) {
				...
				$.ajax({
					...
					url:'processData.php',
					dataType:'json',
					data:{id:_id,for_action:'delete',table:'willdev_redeem','action':'multi_action'},
					success:function(res){
						console.log(res);
						$('.notifyjs-corner').empty();
